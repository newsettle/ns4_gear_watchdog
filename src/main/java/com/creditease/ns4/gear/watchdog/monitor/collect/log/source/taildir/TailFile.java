...
/**
 * @author outman
 * @description 扩展TailFile支持lineNum、IP以及Event大小控制
 * @date 2019/3/7
 */
public class TailFile {
    ...
    static {
        HOST_IP = IPUtil.getHostAddress(true, false);
    }
    ...
    private Event readEvent(boolean backoffWithoutNL, boolean addByteOffset) throws IOException {
        ...
        if (backoffWithoutNL && !line.lineSepInclude) {
            logger.info("Backing off in file without newline: "
                    + path + ", inode: " + inode + ", pos: " + raf.getFilePointer() +", lineNum："+ lineNum);
        ...
        event.getHeaders().put(TIMESTAMP_HEADER_KEY, String.valueOf(System.currentTimeMillis()));
        event.getHeaders().put(HOST_HEADER_KEY, HOST_IP);
        event.getHeaders().put(ABSOLUTE_FILE_PATH_HEADER_KEY, path);
        event.getHeaders().put(OFFSET_HEADER_KEY, posTmp.toString());
        event.getHeaders().put(LINE_NUM_HEADERY_KEY, String.valueOf(++lineNum));
        return event;
    }
